...
static CLWinHandle gSpectralWin;
static bool gSpectralWinInit=false;
static CLFFTPlan gSpectralWinPlan;
static bool gSpectralWinPlanInit=false;

inline void _spectral_win_ensure()
  {
   if(!gSpectralWinInit)
     {
      CLWinReset(gSpectralWin);
      gSpectralWinInit=true;
     }
  }

inline void _spectral_win_plan_ensure()
  {
   if(!gSpectralWinPlanInit)
     {
      CLFFTReset(gSpectralWinPlan);
      gSpectralWinPlanInit=true;
     }
  }

inline void spectral_window_cleanup()
  {
   if(gSpectralWinInit)
     {
      CLWinFree(gSpectralWin);
      CLWinReset(gSpectralWin);
      gSpectralWinInit=false;
     }
   if(gSpectralWinPlanInit)
     {
      CLFFTFree(gSpectralWinPlan);
      CLFFTReset(gSpectralWinPlan);
      gSpectralWinPlanInit=false;
     }
  }
...
inline bool CLWinInit(CLWinHandle &h)
  {
   ...
   CLWinReset(h);
   h.ctx=CLCreateContextGPUFloat64("SpectralOpenCLWindows");
...
"#endif\n"
   ...
   if(h.prog==INVALID_HANDLE)
     {
      PrintFormat("SpectralOpenCLWindows: CLProgramCreate failed (err=%d)", GetLastError());
      if(build_log!="") Print("SpectralOpenCLWindows build log:\n", build_log);
      CLWinFree(h);
      return false;
     }
   ...
   for(int i=0;i<Mx;i++)
     {
      ...
      else real=MathCos(order*MathArccos(x));
      if(odd)
         p[i]=Cx(real,0.0);
      else
        {
...
inline bool CLGetWindowParams(const string win,const int Nx,const bool fftbins,const double &params[],double &out[])
  {
   string name=win;
   StringToLower(name);
   _spectral_win_ensure();
   _spectral_win_plan_ensure();
   double p[]; double c[];
   ...
   if(name=="boxcar" || name=="box" || name=="ones" || name=="rect" || name=="rectangular") type=WIN_BOXCAR;
   ...
   else if(name=="chebwin" || name=="cheb")
     {
      ...
      return CLWindowChebwin(gSpectralWinPlan,Nx,at,!fftbins,out);
     }
   else if(name=="taylor")
     {
      ...
      return CLWindowTaylor(gSpectralWin,Nx,nbar,sll,norm,!fftbins,out);
     }

   return CLWindowGenerate(gSpectralWin,type,Nx,!fftbins,p,c,out);
  }
