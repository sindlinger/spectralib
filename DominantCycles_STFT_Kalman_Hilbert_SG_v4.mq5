//+------------------------------------------------------------------+
//| DominantCycles_STFT_Kalman_Hilbert_SG_v4.mq5                        |
...
enum PhaseAlignMode
  {
   PHASE_ALIGN_NONE=0,
   PHASE_ALIGN_INVERT=1,
   PHASE_ALIGN_RESID_HILBERT=2
  };
...
input double InpPhaseK=0.25;          // phase PLL gain (0..1)
input bool   InpUseOpenCL=true;       // use OpenCL for STFT bins
input int    InpCLDevice=0;           // OpenCL device index
input PhaseAlignMode InpPhaseAlign=PHASE_ALIGN_NONE; // align phase to residual
...
double CorrSignAt(const int i,const int W,const double &a[],const double &b[],const int rates_total)
  {
   if(i<0 || i>=rates_total) return 1.0;
   if(i+W-1>=rates_total) return 1.0;
   double sum=0.0;
   for(int j=i;j<i+W;j++)
     {
      double va=a[j];
      double vb=b[j];
      if(va==EMPTY_VALUE || vb==EMPTY_VALUE) continue;
      sum+=va*vb;
     }
   return (sum>=0.0)?1.0:-1.0;
  }

bool HilbertPhaseAt(const int i,const int L,const int delay,const double &src[],const int rates_total,double &phase,double &amp)
  {
   if(L<=1) return false;
   if(i+L-1>=rates_total) return false;
   double im=0.0;
   for(int n=0;n<L;n++)
      im+=g_hilbertCoeff[n]*src[i+n];
   double re=src[i+delay];
   phase=MathArctan2(im,re);
   amp=MathSqrt(re*re+im*im);
   return true;
  }
  }
...
int OnInit()
  {
   ...
   IndicatorSetString(INDICATOR_SHORTNAME,"DominantCycles_STFT_Kalman_Hilbert_SG_v4");
   ...
   for(int i=start;i>=end;i--)
     {
      ...
      for(int k=0;k<kActive;k++)
        {
         ...
         // bandpass resonator using predicted omega
         ...
         if(p>=0)
           {
            ...
            double measPhase=peakPhase[p];
            measPhase=UnwrapAround(measPhase,phasePred);

            if(InpEnableHilbert && hilbertValid[k])
              {
               double ph=UnwrapAround(hilbertPhase[k],phasePred);
               double snrWeight=snr/(snr+1.0);
               double confWeight=g_confState[k];
               double w=InpHilbertBlend*0.5*(snrWeight+confWeight);
               if(w<0.0) w=0.0; if(w>1.0) w=1.0;
               measPhase=(1.0-w)*measPhase + w*ph;
            ...
            // PLL-style phase update to avoid jumps
            double wPhase=InpPhaseK*(snr/(snr+1.0))*(0.5+0.5*g_confState[k]);
            g_phaseState[k]=PhaseUpdatePLL(phasePred,measPhase,wPhase);
            g_confState[k]=MathMin(1.0,g_confState[k]*InpConfDecay + (snr/(snr+1.0)));
            g_missCount[k]=0;
           }
         else
           {
            g_phaseState[k]=phasePred;
      ...
      // phase alignment to residual (optional)
      if(InpPhaseAlign!=PHASE_ALIGN_NONE && g_outRaw[i]!=EMPTY_VALUE)
        {
         if(InpPhaseAlign==PHASE_ALIGN_INVERT)
           {
            int w=MaxInt(16,InpNormWindow);
            double sgn=CorrSignAt(i,w,g_outRaw,g_resid,rates_total);
            g_outMain[i]=g_outRaw[i]*sgn;
           }
         else if(InpPhaseAlign==PHASE_ALIGN_RESID_HILBERT)
           {
            double phiSum, ampSum, phiRes, ampRes;
            if(HilbertPhaseAt(i,g_hilbertW,g_hilbertDelay,g_outRaw,rates_total,phiSum,ampSum) &&
               HilbertPhaseAt(i,g_hilbertW,g_hilbertDelay,g_resid,rates_total,phiRes,ampRes))
              {
               double delta=WrapPhase(phiRes-phiSum);
               g_outMain[i]=ampSum*MathCos(phiSum+delta);
              }
           }
        }
   ...
   if(InpNormMode!=NORM_NONE)
     {
      ...
      for(int i=maxIdx;i>=minIdx;i--)
        {
         double baseVal=(InpPhaseAlign==PHASE_ALIGN_NONE)?g_outRaw[i]:g_outMain[i];
         ...
         double scale=NormScaleAt(i,W,(InpPhaseAlign==PHASE_ALIGN_NONE)?g_outRaw:g_outMain,rates_total);
         if(scale<=1e-12)
           {
            g_outMain[i]=baseVal;
