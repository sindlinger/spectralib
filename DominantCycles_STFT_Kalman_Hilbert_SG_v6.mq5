//+------------------------------------------------------------------+
//| DominantCycles_STFT_Kalman_Hilbert_SG_v6.mq5                        |
...
double NormScaleAt(const int i,const int W,const double &arr[],const int rates_total,const bool useStd)
  {
...
double NormScaleATRAt(const int i,const int W,
                      const double &high[],const double &low[],const double &close[],const int rates_total)
  {
   if(i<0 || i>=rates_total) return 0.0;
   if(i+W-1>=rates_total) return 0.0;
   double sumTR=0.0;
   ...
   for(int j=i;j<i+W;j++)
     {
      ...
      if(j+1<rates_total)
        {
         double pc=close[j+1];
         double a=MathAbs(high[j]-pc);
         double b=MathAbs(low[j]-pc);
         if(a>tr) tr=a;
         if(b>tr) tr=b;
        }
      sumTR+=tr;
...
int OnInit()
  {
   ...
   IndicatorSetString(INDICATOR_SHORTNAME,"DominantCycles_STFT_Kalman_Hilbert_SG_v6");
...
int OnCalculate(const int rates_total,
                ...
                const int &spread[])
  {
   ...
   static bool s_inputsInit=false;
   static int s_lastNormMode=-1;
   static int s_lastNormWindow=-1;
   static double s_lastNormTarget=0.0;
   static int s_lastSTFTWindow=-1;
   static int s_lastMinPeriod=-1;
   static int s_lastMaxPeriod=-1;
   static int s_lastBinOver=-1;
   static int s_lastWindowType=-1;
   static int s_lastDetrend=-1;
   static int s_lastSGDetrendW=-1;
   static int s_lastSGDetrendOrder=-1;
   static int s_lastHilbertLen=-1;
   static double s_lastResonatorPole=0.0;
   static bool s_lastEnableHilbert=false;
   static int s_lastSGSmoothW=-1;
   static int s_lastSGSmoothOrder=-1;
   static int s_lastSmoothMode=-1;
   static bool s_lastEnableSmooth=false;
   static int s_lastKCycles=-1;
   static int s_lastKcand=-1;
   static int s_lastMinPeakSep=-1;
   static bool s_lastUseOpenCL=false;
   static double s_lastPhaseK=0.0;
   static double s_lastIQP=0.0;
   static double s_lastIQR=0.0;

   bool inputsChanged=false;
   if(!s_inputsInit)
     inputsChanged=true;
   if(s_lastNormMode!=(int)InpNormMode) inputsChanged=true;
   if(s_lastNormWindow!=InpNormWindow) inputsChanged=true;
   if(s_lastNormTarget!=InpNormTarget) inputsChanged=true;
   if(s_lastSTFTWindow!=InpSTFTWindow) inputsChanged=true;
   if(s_lastMinPeriod!=InpMinPeriodBars) inputsChanged=true;
   if(s_lastMaxPeriod!=InpMaxPeriodBars) inputsChanged=true;
   if(s_lastBinOver!=InpBinOversample) inputsChanged=true;
   if(s_lastWindowType!=(int)InpWindowType) inputsChanged=true;
   if(s_lastDetrend!=(int)InpDetrendMode) inputsChanged=true;
   if(s_lastSGDetrendW!=InpSGDetrendWindow) inputsChanged=true;
   if(s_lastSGDetrendOrder!=InpSGDetrendOrder) inputsChanged=true;
   if(s_lastHilbertLen!=InpHilbertLen) inputsChanged=true;
   if(s_lastResonatorPole!=InpResonatorPole) inputsChanged=true;
   if(s_lastEnableHilbert!=InpEnableHilbert) inputsChanged=true;
   if(s_lastSGSmoothW!=InpSGSmoothWindow) inputsChanged=true;
   if(s_lastSGSmoothOrder!=InpSGSmoothOrder) inputsChanged=true;
   if(s_lastSmoothMode!=(int)InpSmoothMode) inputsChanged=true;
   if(s_lastEnableSmooth!=InpEnableSmooth) inputsChanged=true;
   if(s_lastKCycles!=InpKCycles) inputsChanged=true;
   if(s_lastKcand!=InpKcand) inputsChanged=true;
   if(s_lastMinPeakSep!=InpMinPeakSeparation) inputsChanged=true;
   if(s_lastUseOpenCL!=InpUseOpenCL) inputsChanged=true;
   if(s_lastPhaseK!=InpPhaseK) inputsChanged=true;
   if(s_lastIQP!=InpIQProcessVar) inputsChanged=true;
   if(s_lastIQR!=InpIQMeasureVar) inputsChanged=true;

   if(inputsChanged)
     {
      fullRecalc=true;
      s_inputsInit=true;
      s_lastNormMode=(int)InpNormMode;
      s_lastNormWindow=InpNormWindow;
      s_lastNormTarget=InpNormTarget;
      s_lastSTFTWindow=InpSTFTWindow;
      s_lastMinPeriod=InpMinPeriodBars;
      s_lastMaxPeriod=InpMaxPeriodBars;
      s_lastBinOver=InpBinOversample;
      s_lastWindowType=(int)InpWindowType;
      s_lastDetrend=(int)InpDetrendMode;
      s_lastSGDetrendW=InpSGDetrendWindow;
      s_lastSGDetrendOrder=InpSGDetrendOrder;
      s_lastHilbertLen=InpHilbertLen;
      s_lastResonatorPole=InpResonatorPole;
      s_lastEnableHilbert=InpEnableHilbert;
      s_lastSGSmoothW=InpSGSmoothWindow;
      s_lastSGSmoothOrder=InpSGSmoothOrder;
      s_lastSmoothMode=(int)InpSmoothMode;
      s_lastEnableSmooth=InpEnableSmooth;
      s_lastKCycles=InpKCycles;
      s_lastKcand=InpKcand;
      s_lastMinPeakSep=InpMinPeakSeparation;
      s_lastUseOpenCL=InpUseOpenCL;
      s_lastPhaseK=InpPhaseK;
      s_lastIQP=InpIQProcessVar;
      s_lastIQR=InpIQMeasureVar;
     }
   ...
   // optional output normalization (explicit, linear)
   if(InpNormMode!=NORM_NONE)
     {
      ...
      double sigma2=0.0;
      bool sigmaInit=false;
      double alpha=0.0;
      if(InpNormMode==NORM_RESID_EWMA)
        {
         alpha=2.0/(W+1.0);
      ...
      for(int i=maxIdx;i>=minIdx;i--)
        {
         ...
         double scale=0.0;
         if(InpNormMode==NORM_OUT_RMS || InpNormMode==NORM_OUT_STD)
           {
            bool useStd=(InpNormMode==NORM_OUT_STD);
            scale=NormScaleAt(i,W,g_outRaw,rates_total,useStd);
           }
         else if(InpNormMode==NORM_RESID_STD)
           {
            scale=NormScaleAt(i,W,g_resid,rates_total,true);
           }
         else if(InpNormMode==NORM_ATR)
           {
            scale=NormScaleATRAt(i,W,high,low,close,rates_total);
           }
         else if(InpNormMode==NORM_RESID_EWMA)
           {
            double r=g_resid[i];
            if(r!=EMPTY_VALUE)
              {
               if(!sigmaInit)
                 ...
                 sigma2=(1.0-alpha)*sigma2 + alpha*r*r;
               scale=MathSqrt(sigma2);
              }
           }
